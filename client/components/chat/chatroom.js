        ...
        <div className="room">
            <Modal visible={visible} width="auto" footer={null} onCancel={()=>{setVisible(false)}}>
                    ...
                    <h2 style={{fontSize:"1.4em",textAlign:"center",cursor:"default"}}>
                        {data.member.find(el=>el._id!=data.me).name}
                    </h2>
                    </Col>
                    <Col span={24}>
                    <h2 style={{fontSize:"1.3em",textAlign:"center",cursor:"default"}}>
                        {data.member.find(el=>el._id!=data.me).display_name}
                    </h2>
                    </Col>
                    <Col span={24} style={{textAlign:"center"}}>
                        <img width="40%" style={{borderRadius:"200px",cursor:"pointer",marginBottom:"5%"}} src={data.member.find(el=>el._id!==data.me).profile_image}/>
                    </Col>
                    <Col span={24} style={{textAlign:"center"}}>
                    <h2 style={{fontSize:"1.2em",textAlign:"center",cursor:"default"}}>
                        {data.member.find(el=>el._id!=data.me).bio?'Bio: ' + data.member.find(el=>el._id!==data.me).bio:' '}
                    </h2>
                    </Col>
                    <Col span={24} style={{textAlign:"center"}}>
                    <h2 style={{fontSize:"1.2em",textAlign:"center",cursor:"default"}}>
                        Year: {data.member.find(el=>el._id!=data.me).year}
                    </h2>
                    </Col>
                    </Row>
                    )
            ...
            <Row>
                ...
                <Col span={16}>
                    <h2 style={{fontSize:"1.4em",cursor:"default"}}>{data && data.type=== 'General'?data.member.find(el=>el._id!==data.me).display_name :data&& data.name}</h2>
                </Col>
                <Col span={4}>
